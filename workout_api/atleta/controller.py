...
async def post(
    ...
    # verificando se já existe atleta cadastrado com mesmo cpf
    """ existe_atleta = (await db_session.execute(
        select(AtletaModel).filter_by(cpf=atleta_in.cpf))
    ).scalars().first()
    
    if existe_atleta:
        raise HTTPException( # sqlalchemy.exc.IntegrityError
...
async def query(db_session: DatabaseDependency) -> Page[AtletaOut]:
    ...
    return paginate(AtletaOut.model_validate(atleta) for atleta in atletas) #antigo fromorm - convertendo de model para schema
...
async def get_cpf(cpf, db_session: DatabaseDependency) -> AtletaOut:
    atleta: AtletaOut = (
